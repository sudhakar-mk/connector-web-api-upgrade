...
function downloadXML(format2, fileName, fileLength) {
    ...
    zip.file(`${fileName}.xml`, format2);
    downloadCount++;

    // Wait until all expected files are added
    if (downloadCount === fileLength) {
        console.log("All files added. Generating ZIP...");

        showConversionCompleted();
        // 👉 Add the log + nonConverted content as a text file before generating
        let combinedLogs = `--- Log Text ---\n${log_text}\n\n--- Not Converted ---\n${nonConverted}`;
        zip.file("conversion_log.txt", combinedLogs);

        zip.generateAsync({ type: "blob" }).then(function(content) {
            const link = document.createElement('a');
            link.href = URL.createObjectURL(content);
            link.download = 'xml-files.zip'; // name of the final zip file
...
function zipFilesReset() {
    zip = new JSZip(); // Reinitialize zip
    downloadCount = 0;
    log_text = ``;
    nonConverted = ``;
}
