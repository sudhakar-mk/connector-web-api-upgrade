      ...
      showConversionError("Please select a Xml file!");
        ...
        reader.onload = function(event) {
            ...
            referenceData = JSON.parse(event.target.result);
             ...
             filePath = referenceData[1].SwiggerJsonPath;
             baseUrl = referenceData[2].BaseUrl;
             webapiListItemFromJson=referenceData[3].webapiListItem;
             applicationName=referenceData[4].RepositoryName;
             CustomSpXmlStractureFromJson=referenceData[5].CustomSpXmlStracture;
...
//FUNCTION THAT LOOPS THE ACTION ,SAVES XMLDOC IN GLOBAL , DISPLAY THE CONTENT 
    ...
    //generateStartXMLVariables
        ...
        function generateStartXMLVariables(action_name,sp_Name, outputVariable) {
            ...
            let jsonObject;
            let xsdTemplate;
            let sp = CustomSpXmlStractureFromJson.find(item => item.Sp_name === sp_Name);
            if (sp) {
             customElements=sp.sp_params;  
    ...
    const preElement = document.getElementById("xmlContent");
    preElement.textContent = "⏳ Conversion Started...";
    preElement.style.color = "#f1c40f"; // Yellow text
  }

  function showConversionCompleted() {
    const preElement = document.getElementById("xmlContent");
    preElement.textContent = "✅ Conversion Completed Successfully!";
    preElement.style.color = "#2ecc71"; // Green text
  }

    function showConversionError(Error) {
    const preElement = document.getElementById("xmlContent");
    preElement.textContent = Error;
    preElement.style.color = "red"; // Green text
  }
